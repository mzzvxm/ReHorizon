    ...
    public partial class GamerPictureManager : EditorControl
    {
                    ...
                    else
                    {
                        ...
                        if (short.TryParse(imageData.Substring(12, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out imageId))
                        {
                            ...
                            catch
                            {
                                try
                                {
                                    // Fallback XboxGamer.pics caso n�o exista mais na Live original
        ...
        private void cmdGetGamerpics_Click(object sender, EventArgs e)
        {
            ...
            if (txtTitleId.Text.Length == 8 && uint.TryParse(txtTitleId.Text, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out titleIdNum))
            {
                ...
                // Scraping Ass�ncrono para obter todas as imagens de uma vez na velocidade da luz!
                System.Threading.ThreadPool.QueueUserWorkItem(state =>
                {
                                ...
                                foreach (Match m in matches)
                                {
                                    ...
                                    if (!usedIds[titleIdString].Contains(imageId))
                                    {
                                        try
                                        {
                                            ...
                                            Thread.Sleep(10); // Pausa leve para UI n�o travar
                                        }
                                        catch { }
                                    }
                                }
                    ...
                    {
                        // Fallback Cl�ssico: Se o Scraping falhar, ele usa o modo antigo com tentativa e erro
            ...
            using (WebClient wc = new WebClient())
            {
                ...
                for (short x = startId; x <= par.EndID; x++)
                {
                    ...
                    if (!usedIds[par.TitleID].Contains(x))
                    {
                        ...
                        {
                            // Fallback pelo XboxGamer.pics caso a imagem n�o exista na MS
                            try
                            {
        ...
        private void cmdSearchForTitle_Click(object sender, EventArgs e)
        {
            ...
            {
                // Como o doSearchTitle do TitleIDFinder j� foi alterado para buscar online, 
                // ele traz tudo sem necessidade de c�digo extra aqui!
